...
4. Reuse existing project components first; then use accessible primitives or a maintained library; implement custom components only when needed. Add a dependency only when it materially reduces complexity or improves accessibility.
5. Extract interaction intent from the selected Design DNA, then rebuild it with the target's own components, styling, and geometry. Never copy reference component styling or geometry.
6. Prefer CSS transitions. Use a motion library only for coordinated timelines, scroll-linked interactions, or other choreography CSS cannot express cleanly; always respect `prefers-reduced-motion` and readability.
7. Validate responsive states, keyboard and focus behavior, contrast, motion preferences, key interactions, and repository-focused checks. For the editorial profile, also validate readable line length, body font size and line height, heading hierarchy, code overflow, captions, semantic tables, reduced motion, and mobile reading.
8. Validate public-discovery architecture where it exists or is in scope: canonical URLs, structured data, RSS, sitemap, and robots guidance. Run performance or Core Web Vitals checks only when the repository provides a safe supported command or a credible local tool; otherwise report them as unverified rather than inferred.
9. Capture representative screenshots of the implementation. Read [references/design-critic-contract.md](references/design-critic-contract.md), write `design-critic.md`, and update `design-score.json` with implementation alignment and one prioritized `highestROI` improvement. Treat scores as review prompts, not objective measurements.
10. When asking the user to review a completed build, start the repository's documented local preview when available, report its URL, and leave it running for review. Do not install dependencies, invent a preview command, or imply a preview exists when it does not.
11. Report files changed, validation performed, selected Design DNA, Design Critic findings, local preview status, and the highest-impact remaining improvement.
